...
const Pages = {
  create: async (data,userDetails) => {
    ...
    try {
      ...
      if(userDetails?.type == 'Administrator User'){
        await db.execute('INSERT INTO logs (title, created_at) VALUES (?, NOW())', [`Page Created By ${userDetails?.data[0].name}`]);
      }else{
        await db.execute('INSERT INTO storelogs (title, storeId, created_at) VALUES (?,?, NOW())', [`Page Created By ${userDetails?.data[0].name}`, userDetails.data[0].storeId]);
      }
  ...
  update: async (id, data,userDetails) => {
    ...
    try {
      ...
      if(userDetails?.type == 'Administrator User'){
        await db.execute('INSERT INTO logs (title, created_at) VALUES (?, NOW())', [`Page Updated By ${userDetails?.data[0].name}`]);
      }else{
        await db.execute('INSERT INTO storelogs (title, storeId, created_at) VALUES (?,?, NOW())', [`Page Updated By ${userDetails?.data[0].name}`, userDetails.data[0].storeId]);
      }
  ...
  delete: async (id,userDetails) => {
    try {
      ...
      if(userDetails?.type == 'Administrator User'){
        await db.execute('INSERT INTO logs (title, created_at) VALUES (?, NOW())', [`Page Deleted By ${userDetails?.data[0].name}`]);
      }else{
        await db.execute('INSERT INTO storelogs (title, storeId, created_at) VALUES (?,?, NOW())', [`Page Deleted By ${userDetails?.data[0].name}`, userDetails.data[0].storeId]);
      }
      return results;
    } catch (err) {
      throw err;
