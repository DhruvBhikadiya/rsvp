...
const Users = {
    create: async (latitude , longitude , data, userDetails) => {
        ...
        try {
            ...
            if (userDetails?.type == 'Administrator User') {
                await db.execute('INSERT INTO logs (title, created_at) VALUES (?, NOW())', [`User Created By ${userDetails?.data[0].name}`]);
            } else {
                await db.execute('INSERT INTO storelogs (title, storeId, created_at) VALUES (?,?, NOW())', [`User Created By ${userDetails?.data[0].name}`, userDetails.data[0].storeId]);
            }
    ...
    update: async (id, data, userDetails) => {
        ...
        try {
            ...
            if (userDetails?.type == 'Administrator User') {
                await db.execute('INSERT INTO logs (title, created_at) VALUES (?, NOW())', [`User Updated By ${userDetails?.data[0].name}`]);
            } else {
                await db.execute('INSERT INTO storelogs (title, storeId, created_at) VALUES (?,?, NOW())', [`User Updated By ${userDetails?.data[0].name}`, userDetails.data[0].storeId]);
            }
    ...
    updateUserStatus: async (id, isActive, userDetails) => {
        ...
        try {
            ...
            if (userDetails?.type == 'Administrator User') {
                await db.execute('INSERT INTO logs (title, created_at) VALUES (?, NOW())', [`User Status Updated By ${userDetails?.data[0].name}`]);
            } else {
                await db.execute('INSERT INTO storelogs (title, storeId, created_at) VALUES (?,?, NOW())', [`User Status Updated By ${userDetails?.data[0].name}`, userDetails.data[0].storeId]);
            }
    ...
    delete: async (id, userDetails) => {
        try {
            ...
            if (userDetails?.type == 'Administrator User') {
                await db.execute('INSERT INTO logs (title, created_at) VALUES (?, NOW())', [`User Deleted By ${userDetails?.data[0].name}`]);
            } else {
                await db.execute('INSERT INTO storelogs (title, storeId, created_at) VALUES (?,?, NOW())', [`User Deleted By ${userDetails?.data[0].name}`, userDetails.data[0].storeId]);
            }
            return results;
        } catch (err) {
            throw err;
